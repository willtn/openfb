...
var openFB = (function () {
    ...
    function login(callback, options) {
        ...
        if (options && options.return_scopes) {
            return_scopes = "&return_scopes=true";
        }
        ...
        loginWindow = window.open(loginURL + '?client_id=' + fbAppId + '&redirect_uri=' + redirectURL +
            '&response_type=token&scope=' + scope + return_scopes, '_blank', 'location=no,clearcache=yes');
            ...
            if (loginCallback) {
                loginCallback({
                    ...
                    authResponse: {
                        ...
                        grantedScopes: obj['granted_scopes'],
                        deniedScopes: obj['denied_scopes']
                    }
                });
     ...
     * Currently, this just simply removes the token.
     * FIXME: Need to completely revoke permissions and disconnect.
     *
     */
